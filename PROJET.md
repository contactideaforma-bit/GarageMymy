...
- **Pages** `/expert` (tableau de bord + bouton « Données de démo » = `lib/expertise/demo.ts`, idempotent : 6 réparateurs PACA avec taux horaires, 7 missions à chaque étape — mission reçue, visite planifiée, chiffrage, rapport à émettre, émis, VEI — avec rapports brouillon/émis, pièces retenues, et le dossier du PDF modèle AE00034914), `/expert/dossiers` (liste, filtres, `DossierExpertForm` avec lecture IA de la carte grise), `/expert/dossiers/[id]` (onglets Dossier · Photos · Documents · Rapport · Pièces, tous montés), `/expert/rapports`, `/expert/pieces`, `/expert/garages`, `/expert/cabinet` (signature à l'écran ou image).
...
### Ajouté v13.28 — Jetons courriers La Poste (compte Maileva commun) + paiement Qonto
- **Décision** : un seul compte Maileva (celui de l'éditeur) ; chaque garage prépaie ses envois en **jetons**. Exception assumée au « tout inclus » : le courrier papier a un coût externe réel. Lettre simple = 1 jeton, recommandé AR = 5, +1 au-delà de 3 feuilles (page adresse comprise). Packs : 10 j. 24 € HT · 50 j. 110 € HT · 100 j. 200 € HT (modifiables dans `lib/jetons.ts` uniquement). Jetons sans expiration.
- **Migration** `supabase/migration_v90.sql` : `jetons_soldes` (check solde ≥ 0), `achats_jetons` (pack, montants, statut en_attente/paye/expire/annule, lien Qonto, `credite`), `jetons_mouvements` (journal), `envois_postaux.jetons / feuilles / rembourse`, `maileva_config.commun`. Fonctions SQL `jetons_debiter` (atomique, exception SOLDE_INSUFFISANT), `jetons_crediter`, `jetons_crediter_achat` (idempotente) — exécutables par la clé service SEULEMENT. RLS lecture owner.
- **Serveur** `lib/qonto.ts` (clé API `QONTO_LOGIN:QONTO_SECRET_KEY`, `POST /v2/payment_links` panier à usage unique, `GET /v2/payment_links/{id}` ; `QONTO_ENV=sandbox` + `QONTO_STAGING_TOKEN` pour tester), `lib/jetonsServeur.ts` (vérification des achats en attente → crédit). Routes `/api/jetons/acheter`, `/api/jetons/verifier`, `/api/jetons/admin` (GET vue éditeur, POST geste / ajustement / « payé par virement »). Le cron `/api/relances-auto` vérifie aussi les paiements oubliés.
- **Envoi** (`/api/courrier-postal/envoyer`) : pages comptées dans le PDF → feuilles → coût ; débit AVANT Maileva, remboursement automatique si l'envoi échoue ; `/suivi` rend les jetons d'un envoi rejeté (une fois). Réglages Maileva réservés à l'éditeur (ligne `commun`), repli env `MAILEVA_*`.
- **UI** `JetonsPanel` (solde, packs, achat avec acceptation CGU, « J'ai payé — vérifier », revérification au retour sur l'onglet, historique), `GuideCourriers` (page + premier envoi), `EnvoiPostalModal` (coût en jetons, solde, blocage + lien recharge, guide au 1er envoi), `/courriers` (jetons, « Comment ça marche ? », réglages Maileva admin), `/admin/jetons` (ventes, jetons en circulation, soldes par garage, crédit manuel).
- **CGU** : article 8 bis « Courriers La Poste et jetons » (`/cgu#jetons`), `BlocLegal` accepte `id`. Mise à jour 26/09/2026 → prévenir les garages 30 jours avant (art. 11).
