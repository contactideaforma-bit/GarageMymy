...
//  HTML aux couleurs de l'appli (aurora : fond #14102b, violet #8b5cf6,
//  rose #ec4899, turquoise #2dd4bf) mais « email-safe » : tableaux,
//  styles en ligne, dégradés simples — pas de classes CSS.
// ============================================================
...
export function emailBienvenueHtml(b: BienvenueInput): string {
  ...
  const ligne = (label: string, valeur: string, mono = false) => `
    <tr>
      ...
      <td style="padding:6px 0;color:${TEXTE};font-size:${mono ? "15px" : "13px"};${mono ? `font-family:ui-monospace,Menlo,Consolas,monospace;letter-spacing:0.5px;color:${TEAL};font-weight:bold;` : ""}">${valeur}</td>
    </tr>`;
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${FOND};padding:24px 12px">
<tr><td align="center">
<table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%">

  <!-- filet aurora -->
  <tr><td style="height:5px;border-radius:14px 14px 0 0;background:${VIOLET};background:linear-gradient(90deg,${VIOLET},${ROSE},${TEAL})"></td></tr>

  <!-- en-tête -->
  <tr><td style="background:${CARTE};padding:28px 32px 8px 32px">
    <div style="font-family:Segoe UI,system-ui,-apple-system,sans-serif">
      <div style="font-size:11px;letter-spacing:3px;text-transform:uppercase;color:${ROSE};font-weight:bold">${esc(SOCIETE.produit)} by ${esc(SOCIETE.editeur)}</div>
      <div style="font-size:26px;font-weight:800;color:${TEXTE};margin-top:6px">Bienvenue à bord 🚗</div>
    </div>
  </td></tr>

  <!-- corps -->
  <tr><td style="background:${CARTE};padding:12px 32px 28px 32px;font-family:Segoe UI,system-ui,-apple-system,sans-serif">
    <p style="color:${TEXTE};font-size:14px;line-height:1.6;margin:0 0 12px">${bonjour}</p>
    <p style="color:${TEXTE_DOUX};font-size:14px;line-height:1.6;margin:0 0 18px">
      Le compte de <b style="color:${TEXTE}">${esc(b.garageNom)}</b> vient d'être créé sur ${esc(SOCIETE.produit)},
      votre plateforme de gestion des dossiers de sinistres${b.formule ? ` (formule <b style="color:${TEXTE}">${esc(b.formule)}</b>${b.heures ? `, ${b.heures} h de secrétariat par mois` : ""})` : ""}.
    </p>

    <!-- identifiants -->
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${FOND};border:1px solid ${VIOLET}55;border-radius:12px">
      <tr><td style="padding:16px 20px">
        <div style="color:${ROSE};font-size:12px;font-weight:bold;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px">Vos identifiants</div>
        <table role="presentation" cellpadding="0" cellspacing="0">
          ${ligne("Adresse de connexion", `<a href="${esc(url)}" style="color:${TEAL};text-decoration:none">${esc(url.replace(/^https?:\/\//, ""))}</a>`)}
    ...
    <!-- bouton -->
    <table role="presentation" cellpadding="0" cellspacing="0" style="margin:22px auto"><tr><td style="border-radius:10px;background:${VIOLET};background:linear-gradient(90deg,${VIOLET},${ROSE})">
    ...
    <!-- premiers pas -->
    <div style="color:${TEAL};font-size:12px;font-weight:bold;text-transform:uppercase;letter-spacing:1px;margin:4px 0 8px">Vos premiers pas</div>
    <table role="presentation" cellpadding="0" cellspacing="0">
  ...
  <!-- pied -->
  <tr><td style="background:${CARTE};border-radius:0 0 14px 14px;padding:16px 32px;border-top:1px solid ${VIOLET}33">
    <div style="font-family:Segoe UI,system-ui,sans-serif;font-size:11px;color:${TEXTE_DOUX};line-height:1.6">
      ...
      SIRET ${esc(SOCIETE.siret)} · <a href="${esc(SOCIETE.site)}" style="color:${TEXTE_DOUX}">${esc(SOCIETE.site.replace(/^https?:\/\//, ""))}</a>
    </div>
  </td></tr>
</table>
</td></tr>
