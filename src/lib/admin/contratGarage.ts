...
export function conditionsParticulieres(v: VenteContrat, p: Parametres): string[] {
  ...
  const lignes = [
    `Formule souscrite : ${t.libelle}${t.heures ? ` — application + ${t.heures} h de déblocage de dossiers par mois` : " — application seule"}.`,
  ...
  lignes.push(`Heure de Adhésion Service hors forfait : ${eur(p.heureHorsForfait)} HT ; heures non consommées reportables à 50 % sur le mois suivant.`);
  ...
  return [
    {
      ...
      texte: `${ed} (SIRET ${SOCIETE.siret}, ${ADRESSE_COMPLETE}) met à disposition du Client, professionnel de la réparation automobile, l'application en ligne ${SOCIETE.produit} (gestion des dossiers de sinistres, devis, factures, encaissements, documents) et, selon la formule, un service de déblocage de dossiers (Adhésion Service) assuré par des collaborateurs indépendants sous la coordination d'${ed}. Le présent contrat est conclu entre professionnels ; le Client reconnaît souscrire pour les besoins de son activité.`,
    },
    {
...
(c) d'une INDEMNITÉ FORFAITAIRE DE RÉSILIATION égale à CINQUANTE POUR CENT (50 %) des mensualités hors taxes restant à courir jusqu'au terme, au tarif engagé. Les parties conviennent expressément que ce taux représente une évaluation raisonnable du préjudice d'${ed} (perte de marge, coûts d'acquisition et d'installation non amortis, réorganisation du service de déblocage de dossiers (Adhésion Service)), compte tenu du fait que la prestation cesse d'être fournie.
...
2.8 APRÈS L'ENGAGEMENT — au terme des douze mois, le contrat se poursuit par tacite reconduction par périodes d'un mois, sans engagement, au tarif engagé, sauf dénonciation avec un mois de préavis.`,
    ...
    },
    {
      ...
      texte: `À défaut de paiement d'une mensualité dans les quinze jours suivant une relance restée sans effet, ${ed} peut suspendre l'accès à l'application et le service de déblocage de dossiers (Adhésion Service) jusqu'à régularisation, sans que le Client puisse prétendre à indemnité ; les mensualités continuent de courir pendant la suspension. Après trente jours d'impayé, ${ed} peut résilier le contrat aux torts du Client (art. 2). Les données du Client restent exportables pendant 90 jours après la fin du contrat.`,
    },
    {
      titre: "Article 6 — Mise en service et service de déblocage de dossiers (Adhésion Service)",
      texte: `La mise en service comprend le paramétrage du compte, l'import des données transmises par le Client et une formation à distance. Le service de déblocage de dossiers (Adhésion Service) est fourni dans la limite des heures incluses dans la formule, du lundi au vendredi aux horaires ouvrés ; les heures non consommées sont reportables à hauteur de 50 % sur le mois suivant ; les heures supplémentaires sont facturées au tarif hors forfait. Le chargé de mission agit sur instruction du Client, qui reste seul responsable des décisions de gestion, des devis et des factures émis en son nom.`,
    },
    {
